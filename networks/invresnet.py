...
# class InvResNet(nn.Module):
#
#     def __init__(self, block, layers, output_size=256, output_channels=3, input_dims=99, layer_normalization='none', spectral_norm=False):
#             super(InvResNet, self).__init__()
#             self.layer_normalization = layer_normalization
#             self.with_spectral_norm = spectral_norm
#             if self.with_spectral_norm:
...
class SiameseHead(InvResNet):
    def __init__(self, block, layers, output_size=256, output_channels=3, input_dims=99, layer_normalization='none'):
        super(SiameseHead, self).__init__(block, layers, output_size, output_channels, input_dims, layer_normalization)
        self.inplanes = 512
        self.layer1 = self._make_layer(block, 256, layers[0], stride=2)
        self.conv1 = nn.ConvTranspose2d(1024, 512, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv2 = nn.ConvTranspose2d(512, 512, kernel_size=1, stride=1, padding=0, bias=False)
...
class SiameseDecoder(InvResNet):
    def __init__(self, block, layers, output_size=256, output_channels=2, input_dims=99, layer_normalization='batch'):
        super(SiameseDecoder, self).__init__(block, layers, output_size, output_channels, input_dims, layer_normalization)
        self.fc = nn.Linear(input_dims*2, 512)
