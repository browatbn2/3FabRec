...
def create_interpolated_vectors(v1, v2, num, mode='real2real'):
    assert num >= 2
    if mode == 'real2real':
        st = v1
        nd = v2
    ...
    vector_dims = len(v1)
    z_new = torch.zeros((num, vector_dims)).float()
    for i in range(num):
        z_new[i] = st + (nd - st)/(num-1) * i
    return z_new
...
class AAETraining(object):
    ...
    def __init__(self, datasets, args, session_name='debug', snapshot_dir=cfg.SNAPSHOT_DIR,
        ...
        self.optimizer_Q = optim.Adam(Q_params, lr=args.lr, betas=betas)
        self.optimizer_P = optim.Adam(self.saae.P.parameters(), lr=args.lr, betas=betas)
    ...
    def _create_weighted_cross_entropy_loss(self, affectnet_dataset):
    ...
    def update_gan_old(self, X_target, X_recon, z_sample, train=True, with_gen_loss=False, w_gen=0.25, X_gen=None):
        iter_stats = {}
        ...
        if with_gen_loss:
            ...
            # z_noise = self.enc_rand(len(z_sample), z_sample.shape[1]).to(device)
            rand_ids = shuffle(range(len(z_sample)))
        ...
        if  self.iter_in_epoch % UPDATE_DISCRIMINATOR_FREQ == 0:
            ...
            if with_gen_loss:
                ...
                iter_stats.update({'loss_D_rec': loss_D.item(), 'loss_D_gen': loss_D_gen.item()})
            if train:
                loss_D.backward()
        ...
        # update E
        if self.iter_in_epoch % UPDATE_ENCODER_FREQ == 0:
            self.saae.D.zero_grad()
    ...
    def visualize_interpolations(self, Z, nimgs=1, ninterp=8, target_id=-1, wait=10):
        ...
        for st in range(nimgs):
            ZI = create_interpolated_vectors(Z[st], z_morph_target, num=ninterp, mode='real2real').cuda()
...
def add_arguments(parser, defaults=None):
    ...
    # training
    parser.add_argument('--train-encoder', type=bool_str, default=True, help='include encoder update in training ')
    parser.add_argument('--train-decoder', type=bool_str, default=True, help='include decoder update in training ')
